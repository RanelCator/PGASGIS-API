...
let SqlService = SqlService_1 = class SqlService {
    config;
    pool;
    logger = new common_1.Logger(SqlService_1.name);
    constructor(config) {
        this.config = config;
    }
    async onModuleInit() {
        try {
            this.pool = await new sql.ConnectionPool({
                database: this.config.get('SQLSERVER_DB'),
                server: this.config.get('SQLSERVER_HOST'),
                options: {
                    encrypt: true,
                ...
                authentication: {
                    type: 'ntlm',
                    options: {
                        domain: this.config.get('SQLSERVER_DOMAIN') || '',
                        userName: this.config.get('SQLSERVER_USER') || '',
                        password: this.config.get('SQLSERVER_PASS') || '',
                    },
                },
            }).connect();
